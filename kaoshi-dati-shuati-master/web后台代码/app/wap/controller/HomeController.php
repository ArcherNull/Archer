...
/*  		if(checkToken($uid,$token) ==700){
	...
	function msgpost(){       
		...
		if($check){
            ...
            $config = Db::name('option')->field('option_value')->where(['option_name'=>'site_info'])->find();
            $config = json_decode($config['option_value'],true);
            $incScore = $config['user_day_score'] ?? 0;

            $is_inc_score  = db('scores_record')
                ->where('uid', $uid)
                ->where('reason', 3)
                ->where('addtime', 'between', [$today_start, $today_end])->find();

            //如果此文章未评论
            $is_comment = db('comments')->where('uid', $uid)
                         ->where('nid', $id)
                        ->where('addtime', 'between', [$today_start, $today_end])->select();

            if (count($is_comment) < 2) {
                db('users')->where('id', $uid)->setInc('score', $incScore);
                $recordInsert = [
                    'uid' => $uid,
                    'change_score' => $incScore,
                    'reason' => 3,
                    'type' => 1,
                    'addtime' => time()
                ];

                db('scores_record')->insert($recordInsert);
                $this->success('积分+' . $incScore);
            }

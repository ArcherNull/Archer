...
//		}
    ...
    //处理用户每日积分
    function handleUserScoreDay($uid, $type, $infoId = '') {
        //当天结束的时间戳
        $today_end=mktime(0,0,0,date('m'),date('d')+1,date('Y'))-1; //23:59:59
        $expiretime = $today_end - time(); //剩余时间
        ...
        //第一次登录
        if ($type == 1) {
            // 格式为 firstlogin_用户id_20210325
            $key = 'firstlogin_' . $uid . '_' . (string)(date('Ymd'));
            if(!getcaches($key)) {
                setcaches($key, '1', $expiretime);
                $reason = 1;
            }
        ...
        if ($type == 2) {
            if ($infoId != '') {
                //格式 firstread_用户id_20210325
                $key = 'firstread_' . $uid . '_' . $infoId . (string)(date('Ymd'));
                if (!getcaches($key)) {
                    setcaches($key, '1', $expiretime);
                    $reason = 2;
                }
        ...
        if ($type == 4) {
            if ($infoId != '') {
                //格式 firsttest_用户id_20210325
                $key  = 'firsttest_' . $uid . '_' . $infoId . (string)(date('Ymd'));
                if (!getcaches($key)) {
                    setcaches($key, '1', $expiretime);
                    $reason = 4;
                }
        ...
        //加分
        if (($add_score > 0) && ($reason > 0)) {

            $type = 1;
            if ($reason == 5) { //商城兑换是扣除积分
                $type = 2;
            }

            DI()->notorm->users->where(["id"=>$uid])->update(['score' => new \NotORM_Literal("score + {$add_score}" )]);
            //积分记录表
            $day_insert = [
                'uid' => $uid,
                'change_score' => $add_score,
                'reason' => $reason,
                'addtime' => time(),
                'type' => $type
            ];

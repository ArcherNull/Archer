...
class Model_User extends PhalApi_Model_NotORM {
	...
	/****用户待缴费规则详情 */
	public function getPartyRulesDetail($id){
		$info=DI()->notorm->party_pay_rules
					->where('id=?',$id)
					->fetchOne();
		return $info;
	}
	...
	/****用户待缴费记录 */
	public function getPartyRulesList(){
		$list=DI()->notorm->party_pay_rules
					->order('date desc')
					->fetchAll();
		return $list;
	}
	...
	//用户缴费记录详情
	public function getPartyDetail($uid,$ruleid){
		$info=DI()->notorm->party_pay
					->where('uid=? and ruleid=? and status=1',$uid,$ruleid)
					->fetchOne();
		return $info;
	}

	//用户缴费记录详情
	public function getPartyList($uid,$date){
		$info=DI()->notorm->party_pay
					->where('uid=? and status=1 and year=?',$uid,$date)
					->fetchOne();
		return $info;
	}

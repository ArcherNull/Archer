 ...
 * @LastEditTime: 2024-07-31 17:52:39
 * @Description: 字典表
 */
...
class DictionaryController {
  ...
  */
   ...
   * @returns {Promise<void>}
   */
  static async getMultiDictItemsByValues(ctx) {
    //接收客服端参数
    const bodyData = ctx.request.query;
    const { values } = bodyData

    if (values) {
      const valueArr = parseSeqSqlIds(values, 'dicValue');
      if (valueArr.length) {
        const dictList = await models.info_dict.findAll({
          where: {
            [Op.or]: valueArr,
        ...
        const list = await models.info_dict.findAll({
          where: {
            [Op.or]: dicBindIdArr,
            [Op.and]: {
              dicState: 1,
        ...
        const result = {}
        const newList = newDicValue.map(ele => {
          if (ele.dicValue && ele.id) {
            result[ele.dicValue] = list.filter(item => item.dicBindId === ele.id)
          }
        })

        ctx.body = new SuccessModel(result);
      } else {
        ctx.body = new ErrorModel("values参数解析失败");
      }
    } else {
      ctx.body = new ErrorModel("参数values是必传的");
    }
  }

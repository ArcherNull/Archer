...
function formatDate(date) {
	...
	return `${year}-${fd(month)}-${fd(day)} ${fd(hour)}:${fd(minute)}:${fd(seconds)}`;
}
...
export function getPageList() {
	...
	if (isNotEmptyArr(pages)) {
		pages.forEach(ele => {
			...
			if (path) {
				pageObj[path] = ele?.style?.navigationBarTitleText || '未知界面'
			}
		})
	}

	if (isNotEmptyArr(subPackages)) {
		subPackages.forEach(ele => {
			...
			if (isNotEmptyArr(pages)) {
				pages.forEach(item => {
					...
					const nPath = `${name}/${path}`
					if (path) {
						pageObj[nPath] = item?.style?.navigationBarTitleText || '未知界面'
					}
				})
			}
		})

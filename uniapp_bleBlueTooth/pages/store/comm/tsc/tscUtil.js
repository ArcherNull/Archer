...
export const printerByTSC = {
	...
	data: {
		...
		canvasOutId: 'canvasOut',
		canvasJPGId: 'canvasJPG',
	},
	...
	initPhoto: function() {
		...
		const outId = that.data.canvasOutId
		const jpgId = that.data.canvasJPGId
		const ctx_out = uni.createCanvasContext(outId, this);
		...
		//创建一个jpg格式图片
		const ctx_jpg = uni.createCanvasContext(jpgId, this);
	...
	labelTest(device) {
		...
		that.data.isLabelSend = true
		that.prepareSend(device, command.getData())
	},
	...
	printPhoto: function(device) {
		var that = this;
		var canvasWidth = that.data.canvasWidth
		var canvasHeight = that.data.canvasHeight
		const cId = that.data.canvasId
		var command = tsc.jpPrinter.createNew()
		command.setCls()
		...
		uni.canvasGetImageData({
			...
			success: function(res) {
				...
				command.setPrint(1)
				that.prepareSend(device, command.getData()) //发送数据
			},
	...
	prepareSend(device, buff) {
		console.log('准备发送', device, buff)
		//console.log(buff)
		var that = this
	...
	//分包发送
	async Send2(device, buff) {
		console.log('分包发送', device)
		const that = this

		const recSend = (device, buff) => {
					...
					uni.writeBLECharacteristicValue({
						...
						success: async function(res) {
							...
							if (currentTime <= loopTime) {
								...
								await that.recSend(device, buff)
							} else {
								if (currentPrint == printNum) {
	...
	onReady() {
		...
		that.data.buffSize = list
		that.data.oneTimeData = list[0]
		that.data.printNum = numList
		that.data.printerNum = numList[0]
		// that.initPhoto();
	}

<template>
  <div class="ParsingExcel" @click.stop="">
    ...
    >
      请拖拽Excel文件到这，或者
      <BLButton :loading="loading" class="dk-ml-10" type="primary" @click="handleUpload"> 点击上传 </BLButton>
    </div>
...
import { InitColumnDefs, AgGridUtils } from '@/components/AgGridTable/common/agGrid-utils'
...
export default {
  ...
  props: {
    ...
    // 解析方式 ，上传到服务器解析 uploadAndParse  前端解析frontParse
    parseType: {
      type: String,
      default: 'uploadAndParse'
    }
  ...
  methods: {
    ...
    async onSuccess({ results, header }) {
      ...
      const currentFields = await initColumnDefs.judgeReadServerFieldsData(this.fieldsConfig)
    ...
    handleDrop(e) {
      ...
      console.log('this.parseType123123123', this.parseType)
      if (this.parseType === 'uploadAndParse') {
        console.log('上传源文件rawFile', rawFile)
      } else {
        this.upload(rawFile)
      }
      e.stopPropagation()

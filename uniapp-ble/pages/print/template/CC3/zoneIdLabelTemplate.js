/** 深圳战区客制化标签（zoneId=23）来源：Common.getZoneIdLabelTemplate */
...
export function getZoneIdLabelTemplate(data) {
		...
		labelTemplate += `SETMAG 0 0\r\n`
		labelTemplate += `TEXT 3 0 330 235 ${getVal("transitMode")}\r\n` // 产品名称
		labelTemplate += `SETMAG 2 2\r\n`
		...
		labelTemplate += `TEXT 0 20 120 575 品名：${getVal("itemNames")}\r\n`
		labelTemplate += `TEXT 0 20 120 605 件数：${getVal("quantity")}\r\n`
		labelTemplate += `TEXT 0 20 120 635 包装：${getVal("packUnits")}\r\n`
		labelTemplate += `TEXT 0 20 310 575 交接方式：${getVal("handoverMode")}\r\n`
		labelTemplate += `TEXT 0 20 310 605 ${getVal('weight') ?  getVal('weight') + 'KG' : ''}/${getVal("volume") ? getVal("volume") + 'F' : ''}\r\n`
		labelTemplate += `TEXT 0 20 310 635 付款方式：${getVal("paymentMethod")}\r\n`

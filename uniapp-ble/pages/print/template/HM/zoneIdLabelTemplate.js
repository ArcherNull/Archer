/**
 * 汉印 — 深圳战区标签（zoneId=23）
 * 布局对齐 CC3/zoneIdLabelTemplate，指令走 PrinterCpcl
 */
...
export function getZoneIdLabelTemplate(data) {
	...
	return buildCpcl((P) => {
		...
		setMag(P, 0, 0)
		text(P, 3, 0, 330, 235, getVal('transitMode'))
		setMag(P, 2, 2)
		text(P, 3, 0, 15, 300, '收')
		setMag(P, 0, 0)
		...
		text(P, 0, 20, 120, 575, `品名：${getVal('itemNames')}`)
		text(P, 0, 20, 120, 605, `件数：${getVal('quantity')}`)
		text(P, 0, 20, 120, 635, `包装：${getVal('packUnits')}`)
		text(P, 0, 20, 310, 575, `交接方式：${getVal('handoverMode')}`)
		text(
			P,
			0,
			20,
		...
		text(P, 0, 20, 310, 635, `付款方式：${getVal('paymentMethod')}`)

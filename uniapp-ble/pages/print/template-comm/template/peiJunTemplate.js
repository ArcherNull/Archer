/**
 * CPCL 配军运单/回单模板 — CC3 / 汉印共用
 * 纸张基准：700×800（0.1mm），布局对齐 Archer buildTemplate2
 * 指令以芝柯风格生成，汉印经 applyBrandDialect 转换
 */

import { resolveBrand } from './_helpers.js'

/** 将芝柯风格 CPCL 转为汉印方言（避免多走纸：去掉 GAP-SENSE+FORM） */
function applyBrandDialect(cpcl, brand) {
	const b = String(brand || 'common').toUpperCase()
	if (b !== 'HM' && b !== 'HPRT') return cpcl
	let s = String(cpcl || '')
	s = s.replace(/\bPAGE-WIDTH\b/g, 'PW')
	s = s.replace(/(^|\r?\n)T /g, '$1TEXT ')
	s = s.replace(/(^|\r?\n)L /g, '$1LINE ')
	s = s.replace(/(^|\r?\n)B QR/g, '$1BARCODE QR')
	s = s.replace(/(^|\r?\n)B /g, '$1BARCODE ')
...
export function peijunTemplateJson(params = {}, options = {}) {
	...
	console.log('配军模版接收', params, brand)
	...
	return applyBrandDialect(lines.join('\n') + '\n', brand)
}

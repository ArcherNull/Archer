/**
 * @file template-comm/template/jcLabelTemplate.js
 * @desc 军城旧标签 — CC3 / 汉印共用
 */
 ...
 */
export function buildJCLabelTemplate(data = {}, options = {}) {
	const json = data || {}
	...
	b.setMag(0, 0)
	b.setMag(2, 2)
	b.text(3, 0, 330, 220, getVal('transitMode'))
	b.setMag(0, 0)
	b.setMag(2, 2)
	b.text(3, 0, 15, 300, '收')
	b.setMag(0, 0)
	...
	b.text(3, 0, 120, 565, `品名：${getVal('itemNames')}`)
	b.text(3, 0, 120, 600, `件数：${getVal('quantity')}`)
	b.text(3, 0, 120, 635, `包装：${getVal('packUnits')}`)
	b.text(0, 20, 310, 565, `交接方式：${getVal('handoverMode')}`)
	b.text(
		3,
		0,
		310,
	...
	b.text(0, 20, 310, 640, `付款方式：${getVal('paymentMethod')}`)
...
export function getJCLabelTemplate(data, options) {
	return buildJCLabelTemplate(data, options).cpcl
}

 ...
 * @return {*}
 */
export function getColumnDefs(options: any, operationEle?: any) {
            ...
            if (label === '操作' && prop === 'operation') {
              ...
              if (operationEle) {
                operationColumns.cellRenderer = defineComponent((props) => {
                  const { data, rowIndex, value } = props.params;
                  const scope = {
                    index: rowIndex,
                    row: data,
                    value,
              ...
              if (cellStyle && isObject(cellStyle) && !isEmpty(cellStyle)) {
              ...
              if (isNotEmptyArr(_children)) {
                if (headerStyle) {
                  const cHStyle = headerStyle ?? {};
                  dObj.headerStyle = Object.assign(
                    {
                      color: 'red',
                      'text-align': 'center',
                    },

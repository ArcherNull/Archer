      ...
      <div class="w-320 flex-col px-20 py-32">
        ...
        <div class="mt-20 flex items-center">
          ...
          <img
            ...
            alt="验证码"
            height="40"
            class="ml-12 h-40 w-80 cursor-pointer"
            @click="initCaptcha"
          >
        </div>
...
const initCaptcha = throttle(async () => {
  const res = await api.getCaptchaUrl({ type: 'login' })
...
function quickLogin() {
  loginInfo.value.email = '991584844@qq.com'
...
async function handleLogin() {
  ...
  try {
    ...
    const { data } = await api.login({ email, password: password.toString(), captcha })
    if (isRemember.value) {

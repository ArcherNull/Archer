...
UPLOAD_PREFIX = 'static/upload/profile'
UPLOAD_PATH = 'static/upload/upload_path'
UPLOAD_MACHINE = 'A'
...
DOWNLOAD_PATH = 'static/upload/download_path'
...
class UploadUtil:
    ...
    def check_file_extension(cls, file: UploadFile):
        ...
        if file_extension in DEFAULT_ALLOWED_EXTENSION:
            return True
        return False
    ...
    def check_file_timestamp(cls, filename: str):
        ...
        :return: 校验结果
        """
        timestamp = filename.rsplit('.', 1)[0].split('_')[-1].split(UPLOAD_MACHINE)[0]
        try:
    ...
    def check_file_machine(cls, filename: str):
        ...
        :return: 校验结果
        """
        if filename.rsplit('.', 1)[0][-4] == UPLOAD_MACHINE:
            return True
        return False

...
<template>
  ...
  >
    <n-card :style="modalOptions.contentStyle" :closable="modalOptions.closable" @close="close()">
      ...
      <n-spin :show="contentStatus === 'loading'" size="small">
        <div v-if="contentStatus === 'empty'" class="cs-content">
          <MeEmpty />
        </div>
        <div v-else-if="contentStatus === 'error'" class="cs-content">
          <MeResult status="error" />
        </div>
        <div v-else-if="contentStatus === 'success'">
          <slot />
        </div>
...
<script setup>
import { MeEmpty, MeResult } from '@/components/index'

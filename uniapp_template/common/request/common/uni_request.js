...
export default function ({ baseURL, timeout = 56 * 1000, header: headers, statusCode = [200, 401] }) {
	return {
		...
		onerror: () => {uni.showToast({ title:'网络异常', icon:'error' })}, // 请求错误钩子函数集合
		file(method, rUrl, data, header, reqIntercept, resIntercept) {
			const url = this.getBaseUrl(rUrl)
		...
		request(method, rUrl, data, header, reqIntercept, resIntercept) {
			const url = this.getBaseUrl(rUrl)
			let timer, // timer 检测超时定时器
				requestTask, // requestTask 网络请求 task 对象
				aborted = false, // aborted 请求是否已被取消
		...
		// 校验HTTP/HTTPS/WS/WSS链接
		validateUrl(url) {
			// 方法1：使用正则表达式
			const regex = /^(http|https|ws|wss):\/\/[\w.-]+(?:\.[\w.-]+)+[\w\-._~:/?#[\]@!$&'()*+,;=]+$/;
			...
			if (!this.validateUrl(url)) {
				if (this.baseURL) {
					if (typeof this.baseURL === 'string') {
						newUrl = url[0] === '/' ? this.baseURL + url : url
					} else if (typeof this.baseURL === 'object' && Object.keys(this.baseURL)?.length) {
						const splitArr = url.split('/').filter(Boolean)
						const [perfix, ...restApiArr] = splitArr
						const apiPerfix = `/${perfix || ''}`
						const proxy_domain = CONFIG.APP_DOMAIN_PROXY[apiPerfix] || ''
						let domain, rUrl

						// 如果获取到代理域名，则切割url的前缀；如果没有获取到，则不切割
						if (proxy_domain) {
							domain = proxy_domain
							rUrl = restApiArr.length ? `${domain.endsWith('/') ? '' : '/'}${restApiArr.join('/')}` : ''
						} else {
							domain = CONFIG.APP_DOMAIN_PROXY['/']
							rUrl = url
						}

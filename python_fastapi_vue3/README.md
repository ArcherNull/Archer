...
# 创建python的项目环境
python -m venv ruoyienv

# 激活虚拟环境，windows执行
ruoyienv\Scripts\activate

# 激活虚拟环境，对于macOS和Linux中执行
source ruoyienv/bin/activate
...
# 构建生产环境
npm run build:ele 或 yarn build:ele
```
...
# 执行sql文件夹中的 ruoyi-fastapi-pg.sql / ruoyi-fastapi.sql 文件
...
# 将前端代码启动起来后，访问以下网址可以访问接口文档
http://localhost:8008/dev-api/docs

http://localhost:8008/dev-api/redoc
```

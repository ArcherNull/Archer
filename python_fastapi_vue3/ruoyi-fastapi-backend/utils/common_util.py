...
class SqlalchemyUtil:
    ...
    @classmethod
    def base_to_dict(cls,
                     obj: Union[Base, Dict],
                     transform_case: Literal['no_case', 'snake_to_camel',
                                             'camel_to_snake'] = 'no_case'):
        """
        ...
        if isinstance(obj, Base):
            ...
            for name, value in base_dict.items():
                if isinstance(value, InstrumentedList):
                    base_dict[name] = cls.serialize_result(
                        value, 'snake_to_camel')
        elif isinstance(obj, dict):
            base_dict = obj.copy()
        if transform_case == 'snake_to_camel':
            return {
                CamelCaseUtil.snake_to_camel(k): v
                for k, v in base_dict.items()
            }
        elif transform_case == 'camel_to_snake':
            return {
                SnakeCaseUtil.camel_to_snake(k): v
                for k, v in base_dict.items()
            }
    ...
    def serialize_result(
        ...
        if isinstance(result, (Base, dict)):
            ...
            return [
                cls.serialize_result(row, transform_case) for row in result
            ]
        elif isinstance(result, Row):
            if all([isinstance(row, Base) for row in result]):
                return [
                    cls.base_to_dict(row, transform_case) for row in result
                ]
...
class CamelCaseUtil:
    ...
    def transform_result(cls, result: Any):
        ...
        :return: 小驼峰形式结果
        """
        return SqlalchemyUtil.serialize_result(result=result,
                                               transform_case='snake_to_camel')
...
def get_excel_template(header_list: List, selector_header_list: List,
...
def get_filepath_from_url(url: str):
    ...
    filepath = os.path.join(CachePathConfig.PATH, task_path, task_id,
                            file_name)

 ...
 * @LastEditTime: 2024-12-17 19:11:31
 * @Description: https://www.ag-grid.com/javascript-data-grid/
 */

import { cloneDeep, isArray, isEmpty, isObject, sumBy } from 'lodash-es';

import { showConfirm, showMessage } from '#/comm/utils/index';
...
export function AgGridUtils(api) {
  ...
  const { api: gridApi, colDef, columnApi, rowPinned } = api;
  ...
  if (gridApi || columnApi) {
    ...
    this.getCurrentGridData = function () {
      ...
      return Array.isArray(getCurrentGridNode) && getCurrentGridNode.length > 0
        ? getCurrentGridNode.map((ele) => ele.data)
        : [];
    };
    ...
    this.allLeafChildren = function () {
      ...
      return Array.isArray(getRootGridData) && getRootGridData.length > 0
        ? getRootGridData.map((ele) => ele.data)
        : [];
    };
    ...
    this.getSelectedRows = function () {
      ...
      if (this.checkRowsType(selectedRowData)) {
    ...
    this.jumpToCol = function (number) {
      ...
      if (Number.isNaN(index)) {
        ...
        if (index > 0) {
          jumpIndex = index <= allColumnsLength ? index : allColumnsLength;
        } else {
          jumpIndex = 1;
    ...
    this.calculateTotalLine = (list) => {
      if (isArray(list) && !isEmpty(list)) {
        ...
        const autoCalc = (ele) =>
          ele === 'numericalOrder' ||
          /Fee|money|Money|Price|Pay|Weight|Volume|Num|costShare|age$/.test(
            ele,
          );
        // 数字转换
        ...
        // 格式化数字
        const formatMathNum = (num) =>
          !num && convertNumber(num) ? 0 : num.toFixed(3);
        ...
        // 平均值计算字段
        const averageField = new Set(['costRate']);
        ...
        const getAverageNum = (arr, keyName) => {
          ...
          return (
            sum &&
            formatMathNum((convertNumber(sum) * 1000) / (1000 * arr.length))
          );
        };

        for (const ele of keys) {
          if (autoCalc(ele)) {
            if (Number.isNaN(Number(val[ele]))) {
              val[ele] = '';
            } else {
              // 判断是不是序号行
    ...
    this.getCurrentGridDataAndNumericalOrder = (getCurrentGridNode = []) => {
      return Array.isArray(getCurrentGridNode) && getCurrentGridNode.length > 0
        ? getCurrentGridNode.map((ele, ind) => {
...
export class InitColumnDefs {
  ...
  init(fieldsConfig, oldServerData) {
    const { columnDefs, database, showFirstColumn, specColumns } = fieldsConfig;
    ...
    if (isEmpty(database)) {
      ...
      if (oldServerData?.length) {
        ...
        if (isEmpty(columnDefs)) {
          let colDefs = [];
          colDefs = isEmpty(specColumns)
            ? this.initColDefs(database)
            : this.dealSpecColumns(database, specColumns);
      ...
      if (isArray(insertColumns) && !isEmpty(insertColumns)) {
        insertColumns.forEach((ele) => {
          ...
          if (isArray(filterItem) && !isEmpty(filterItem)) {
            ...
            console.error(
              `插入列字段名称为【${ele.headerName}-${ele.field}】,插入原列字段【${ele.afterField}】后，未成功`,
            );
            idsArr.push(ele);
          }
        });
  ...
  reloadColumns() {
    // 初始化
    return this.init(this.fieldsConfig, this.oldServerData);
  }

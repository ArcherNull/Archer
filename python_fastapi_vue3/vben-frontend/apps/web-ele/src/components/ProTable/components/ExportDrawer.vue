 ...
 * @LastEditTime: 2025-03-18 16:43:30
 * @Description: 
-->
...
// 处理数据的函数  解决货物时效看板导出问题
const processData = (data) => {
  const newData = { ...data };
  if (Array.isArray(newData.pointIds)) {
    newData.pointIds = newData.pointIds.join(',');
  }
  return newData;
};
...
const exportFileFun = async () => {
  try {
    ...
    const newParams = processData(param);
    console.log('param=====>', JSON.stringify(newParams));
